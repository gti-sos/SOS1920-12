<script>
	...
	async function loadSchoolDropouts(){
		...
		const res = await fetch("/api/v1/school-dropouts/loadInitialData").then(function(res){
			if (res.ok){
				console.log("OK");
				getSchoolDropouts();
	...
	async function getSchoolDropouts(){
		...
		console.log("Fetching school dropouts...");
		var url = "/api/v1/school-dropouts?limit="+limit+"&offset="+(offset*limit);
		var urlAfter = "/api/v1/school-dropouts?limit="+limit+"&offset="+(limit*(offset+1));
	...
	async function insertSchoolDropout(){
		...
		if (newSchoolDropout.country == "" || newSchoolDropout.country == null || newSchoolDropout.year == "" || newSchoolDropout.year == null) {
			...
			const res = await fetch("/api/v1/school-dropouts",{
			method:"POST",
	...
	async function deleteSchoolDropout(country,year) {
		...
		if(confirm("¿Está seguro de que desea eliminar esta entrada?")){
			...
			const res = await fetch("/api/v1/school-dropouts/" + country + "/"+year,{
				method:"DELETE"
			}).then(function (res) {
				if(res.ok){
					getSchoolDropouts();
